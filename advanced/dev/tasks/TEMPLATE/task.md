# Task title

- **Task ID:** `[tracker-id-]kebab-slug` — e.g. `explore-caching` or `PROJ-42-explore-caching`
- **External tracker ID:** <!-- none | PROJ-42 | … -->
- **Status:** backlog
- **Parent / blocked by:** <!-- optional -->

## Problem

Why this work exists. What is wrong, missing, or painful today.

## Goal

One clear outcome. Observable for humans and agents.

## Open questions

Do not implement if any row is `open`. Flip to `locked` when the user accepts. 

| # | Topic | Status | Decision |
|---|--------|--------|----------|
| 1 | … | open | |

## Non-goals

What this task deliberately skips.

## Done when

- [ ] Observable acceptance criteria (build success, tests).
- [ ] Durable docs updated if contracts or user-facing behavior changed (`dev/README.md` / ADR / layer README or other project docs as needed).
- [ ] Anything else.

## Notes

Short pointers only. Extra material for this task lives **alongside** `task.md` in this folder — files or subfolders as needed. Structure is free and can differ per task (implementation details, notes, samples, analysis, assets, etc.).
